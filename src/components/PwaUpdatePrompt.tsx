...
import { subscribePWA, getPWAState, applyPWAUpdate, dismissPWAUpdate } from '../pwa';
...
export const PwaUpdatePrompt: React.FC = () => {
  const [needRefresh, setNeedRefresh] = useState(getPWAState().needRefresh);

  useEffect(() => {
    const unsub = subscribePWA(() => {
      setNeedRefresh(getPWAState().needRefresh);
    });
    return unsub;
  }, []);
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-[80] p-4 flex justify-center pointer-events-none animate-fade-in">
