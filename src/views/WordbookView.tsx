...
export const WordbookView: React.FC<WordbookViewProps> = ({
  ...
  const renderWordStatusBadge = (item: Word) => {
    ...
    if (isFullyPassed) {
      return (
        <span className="text-[10px] font-semibold text-emerald-700 bg-emerald-100 dark:bg-emerald-950/80 dark:text-emerald-300 px-2 py-0.5 rounded-full">
          完全通过
        </span>
      );
    }
    if (isTodayPassed) {
      return (
        <span className="text-[10px] font-semibold text-sky-700 bg-sky-100 dark:bg-sky-950/80 dark:text-sky-300 px-2 py-0.5 rounded-full">
          今日已过关
        </span>
      );
    }
    if (item.count_practiced > 0) {
      return (
        <span className="text-[10px] font-medium text-amber-700 bg-amber-100 dark:bg-amber-950/80 dark:text-amber-300 px-2 py-0.5 rounded-full">
          学习中
        </span>
      );
    }
